...
void odometryHandler(const nav_msgs::Odometry::ConstPtr& odom)
{
  ...
  odomData.pose.pose.position.x = odom->pose.pose.position.x + 0;
  odomData.pose.pose.position.y = odom->pose.pose.position.y + 0;
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudIn)
{
  ...
  if (flipRegisteredScan) {
    ...
    for (int i = 0; i < laserCloudSize2; i++) {
      float temp = laserCloud->points[i].x;
      laserCloud->points[i].x = laserCloud->points[i].x + 0;
      laserCloud->points[i].y = laserCloud->points[i].y + 0;
    }
